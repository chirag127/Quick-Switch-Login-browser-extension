...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if (currentTab && currentTab.url && isValidUrl(currentTab.url)) {
      ...
      currentSiteFavicon.style.backgroundImage = `url(${getFaviconUrl(domain)})`;
      saveSessionButton.disabled = false;
    } else {
      currentSiteDomain.textContent = 'Not a valid page for session management';
    ...
    if (isLoggedIn) {
      ...
      syncMessage.textContent = 'Sign in to sync your sessions across devices';
      syncStatus.classList.remove('hidden');
    }
...
const handleRestoreSession = async (session) => {
  try {
    ...
    if (!currentTab || !currentTab.url || getDomain(currentTab.url) !== session.domain) {
      showNotification(`Please navigate to ${session.domain} to restore this session`, 'error');
      return;
    }
    
    // Confirm restoration
    const confirmed = await confirmAction(`Replace current session for ${session.domain} with "${session.name}"?`);
    ...
    showNotification(`Session "${session.name}" restored successfully`, 'success');
...
const handleDeleteSession = async (session) => {
  try {
    // Confirm deletion
    const confirmed = await confirmAction(`Delete session "${session.name}" for ${session.domain}?`);
    ...
    showNotification(`Session "${session.name}" deleted successfully`, 'success');
